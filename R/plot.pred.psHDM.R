...
plot.pred.psHDM <- function(object,
  ...
  # object <- list.to.df(object = object)
  ...
  if(!is.null(object$pop.level)){
    ...
    aa1 <- ggplot2::ggplot(data = object$plant.obs) +
      ggplot2::geom_line(ggplot2::aes(timeNumber, obs_plant, group = plant, colour = "1")) +
      # ggplot2::geom_line(data = object$plant.level[!is.na(object$plant.level$obs_plant),],
      #                    ggplot2::aes(timeNumber, obs_plant, group = plant, colour = "1")) +
      # ggplot2::geom_point(ggplot2::aes(timeNumber, obs_plant, group = plant, colour = "1"), shape = 20) +
      ggplot2::geom_line(data = object$pop.level, ggplot2::aes(timeNumber, f_pop, group = pop, colour="2")) +
      ggplot2::geom_rug(ggplot2::aes(x = timeNumber, y = NULL), color = "gray", length = ggplot2::unit(0.01, "npc")) +
      ggplot2::scale_x_continuous(breaks = round(seq(min.t, max.t, length.out = 5), 0)) +
      ggplot2::scale_color_manual(values = my.cols, labels = c(expression(tilde(y)[pgi](t)),expression(hat(f)[p](t))))+
      ggplot2::labs(x = xlab, y = ylab, title = global.main$pop.tra, color = "") +
      my.theme() + ggplot2::facet_grid( ~ pop)
    if(!is.null(object$pop.level$se_pop)){
      aa1 <- aa1 + ggplot2::geom_ribbon(data = object$pop.level, ggplot2::aes(x = timeNumber, ymin = f_pop - 1.96 * se_pop, ymax = f_pop + 1.96 * se_pop, group = pop), fill = ggplot2::alpha("blue", 0.3))
    }
    print(aa1)
  }
  ...
  if(!is.null(object$geno.level)){
    ...
    bb1 <- ggplot2::ggplot(data = object$geno.level) +
      ggplot2::geom_line(ggplot2::aes(timeNumber, f_geno, group = geno, colour="1")) +
      ggplot2::geom_line(data = object$pop.level, ggplot2::aes(timeNumber, f_pop, group = pop, colour="2"), size = 0.8) +
      ggplot2::geom_rug(data = object$plant.obs, ggplot2::aes(x = timeNumber, y = NULL), color = "gray", length = ggplot2::unit(0.01, "npc")) +
      ggplot2::scale_x_continuous(breaks = round(seq(min.t, max.t, length.out = 5), 0)) +
      ...
      my.theme() + ggplot2::facet_grid( ~ pop)
    if(!is.null(object$pop.level$se_pop)){
      bb1 <- bb1 + ggplot2::geom_ribbon(data = object$pop.level, ggplot2::aes(x = timeNumber, ymin = f_pop - 1.96 * se_pop, ymax = f_pop + 1.96 * se_pop, group = pop), fill = ggplot2::alpha("orange", 0.5))
    }
    ...
    bb2 <- ggplot2::ggplot(data = object$geno.level) +
      ggplot2::geom_line(ggplot2::aes(timeNumber, f_geno_deriv1, group = geno, colour = "1")) +
      ggplot2::geom_line(data = object$pop.level, ggplot2::aes(timeNumber, f_pop_deriv1, group = pop, colour = "2")) +
      ggplot2::geom_rug(data = object$plant.obs, ggplot2::aes(x = timeNumber, y = NULL), color = "gray", length = ggplot2::unit(0.01, "npc")) +
      ggplot2::scale_x_continuous(breaks = round(seq(min.t, max.t, length.out = 5), 0)) +
      ggplot2::ylim(min(object$geno.level$f_geno_deriv1), max(object$geno.level$f_geno_deriv1)*1.2) +
    ...
    if(!is.null(object$pop.level$se_pop_deriv1)){
      bb2 <- bb2 + ggplot2::geom_ribbon(data = object$pop.level, ggplot2::aes(x = timeNumber, ymin = f_pop_deriv1 - 1.96 * se_pop_deriv1, ymax = f_pop_deriv1 + 1.96 * se_pop_deriv1, group = pop), fill = ggplot2::alpha("orange", 0.5))
    }
    if(ask) readline("Press return for next page....")
    print(bb2)
    ...
    cc1 <- ggplot2::ggplot(data = object$geno.level) +
      ggplot2::geom_line(ggplot2::aes(timeNumber, f_geno_dev, group = geno, colour="1")) +
      ggplot2::geom_rug(data = object$plant.obs, ggplot2::aes(x = timeNumber, y = NULL), color = "gray", length = ggplot2::unit(0.01, "npc")) +
      ggplot2::scale_x_continuous(breaks = round(seq(min.t, max.t, length.out = 5), 0)) +
  ...
  if(!is.null(object$plant.level)) {
    ...
    dd1 <- ggplot2::ggplot(data = df.obs.HDM.sub) +
      ggplot2::geom_line(ggplot2::aes(timeNumber, obs_plant, group = plant, colour = "1")) +
      # ggplot2::geom_line(data = df.obs.HDM.sub[!is.na(df.obs.HDM.sub$obs_plant),], ggplot2::aes(timeNumber, obs_plant, group = plant, colour = "1")) +
      # ggplot2::geom_point(ggplot2::aes(timeNumber, obs_plant, group = plant, colour = "1"), shape = 20) +
      ggplot2::geom_line(data = df.tra.HDM.sub, ggplot2::aes(timeNumber, f_plant, group = plant, colour = "2"), linetype = 2) +
      ggplot2::geom_line(data = df.tra.HDM.geno.sub, ggplot2::aes(timeNumber, f_geno, group = geno, colour = "3")) +
      ggplot2::geom_rug(data = df.obs.HDM.sub, ggplot2::aes(x = timeNumber, y = NULL), color = "gray", length = ggplot2::unit(0.01, "npc")) +
      ggplot2::scale_x_continuous(breaks = round(seq(min.t, max.t, length.out = 5), 0)) +
      ...
      my.theme() + ggplot2::facet_grid( ~ geno)
    if(!is.null(object$geno.level$se_geno)){
      dd1 <- dd1 + ggplot2::geom_ribbon(data = df.tra.HDM.geno.sub, ggplot2::aes(x = timeNumber, ymin = f_geno - 1.96 * se_geno, ymax = f_geno + 1.96 * se_geno, group = pop), fill = ggplot2::alpha("orange", 0.5))
    }
    if(ask) readline("Press return for next page....")
    print(dd1)
  }
