# Help functions ----------------------------------------------------------
  ...
  list.to.df             <- function(object1, object2, what, xp) {
    ...
    if(what == "geno") {
      df.geno.tra <- data.frame(timeNumber = rep(xp, length(object2$l.geno)),
                                pop = rep(object2$l.pop, object2$n.geno_p_pop*length(xp)),
    ...
    if(what == "plant") {
      df.plant.tra <- data.frame(timeNumber = rep(xp, sum(object2$n.plants_p_geno)),
                                 pop = rep(object2$l.pop, object2$n.plants_p_pop*length(xp)),
                                 geno = rep(object2$l.geno, object2$n.plants_p_geno*length(xp)),
      ...
      if(isTRUE(setequal(xp, object2$time))){
        ...
        df.plant.obs <- data.frame(timeNumber = rep(object2$time, sum(object2$n.plants_p_geno)),
                                   pop = rep(object2$l.pop, object2$n.plants_p_pop*length(object2$time)),
                                   geno = rep(object2$l.geno, object2$n.plants_p_geno*length(object2$time)),
...
#' @param newtimes A numeric vector with timepints at which predictions are
#' desired
...
#'                             newtimes = seq(min(fit.psHDM$time), max(fit.psHDM$time), length = 100),
...
predict.psHDM <- function(object,
  ...
  if(isTRUE(pred$pop)) {
    ...
    if(isTRUE(se$pop)) {
      pop_level$pred <- append(pop_level$pred, standard_errors(Tm = pop_level$Tm, B = pop_level$B, B.d1 = pop_level$B.d1, B.d2 = pop_level$B.d2, what = "pop", dev = FALSE, object = object, np.s, np.e))
      print("Standard errors for population-specific growth curves OK")
    }
    # Data frame with all the information at population level
    res <- append(res, list.to.df(object1 = pop_level$pred, object2 = object, what = "pop", xp))
  }

  # Functions at genotype level
  if(isTRUE(pred$geno)) {
    # Genotype-specific deviations and first- and second-order derivatives
      geno_dev <- mixmod_to_bspline_pred(what = "geno", object = object, np.s, np.e, xp, dev = TRUE)
    ...
    # Genotype-specific growth curves and first- and second-order derivatives
      ...
      geno_tra <- mixmod_to_bspline_pred(what = "geno", object = object, np.s, np.e, xp, Tmat = T_geno, mod.mat = mm.geno.pop, dev = FALSE,
                                               Bbasis = list(B = geno_dev$B, B.d1 = geno_dev$B.d1, B.d2 = geno_dev$B.d2))
      geno_level$geno_tra <- geno_tra$pred
      print("Genotype-specific growth curves OK")

      if(isTRUE(se$geno)) {
        # Genotype-specific deviations
        geno_level$geno_dev <- append(geno_level$geno_dev, standard_errors(Tm = geno_dev$Tm, B = geno_dev$B, B.d1 = geno_dev$B.d1, B.d2 = geno_dev$B.d2, what = "geno", dev = TRUE, object = object, np.s, np.e))
        print("Standard errors for genotype-specific deviations OK")

        # Genotype-specific growth curves
        geno_level$geno_tra <- append(geno_level$geno_tra, standard_errors(Tm = T_geno, B = geno_tra$B, B.d1 = geno_tra$B.d1, B.d2 = geno_tra$B.d2, what = "geno", dev = FALSE, object = object, np.s, np.e))
        print("Standard errors for genotype-specific growth curves OK")
      }
      # Data frame with all the information at genotype level
      res <- append(res, list.to.df(object1 = geno_level, object2 = object, what = "geno", xp))
  }

  # Functions at plant level
  if(isTRUE(pred$plant)) {
    # Plant-specific deviations and first- and second-order derivatives
      plant_dev <- mixmod_to_bspline_pred(what = "plant", object = object, np.s, np.e, xp, dev = TRUE)
    ...
    # Plant-specific growth curves
      ...
      plant_tra <- mixmod_to_bspline_pred(what = "plant", object = object, np.s, np.e, xp, Tmat = T_plant, mod.mat = list(mm.ind.pop, mm.ind.geno), dev = FALSE,
                                          Bbasis = list(B = plant_dev$B, B.d1 = plant_dev$B.d1, B.d2 = plant_dev$B.d2))
      plant_level$plant_tra <- plant_tra$pred
      print("Plant-specific growth curves OK")

      if(isTRUE(se$plant)) {
        # Plant-specific deviations
        plant_level$plant_dev <- append(plant_level$plant_dev, standard_errors(Tm = plant_dev$Tm, B = plant_dev$B, B.d1 = plant_dev$B.d1, B.d2 = plant_dev$B.d2, what = "plant", dev = TRUE, object = object, np.s, np.e))
        ...
        plant_level$plant_tra <- append(plant_level$plant_tra, standard_errors(Tm = T_plant, B = plant_tra$B, B.d1 = plant_tra$B.d1, B.d2 = plant_tra$B.d2, what = "plant", dev = FALSE, object = object, np.s, np.e))
        print("Standard errors for plant-specific growth curves OK")
      }
      # Data frame with all the information at genotype level
      res <- append(res, list.to.df(object1 = plant_level, object2 = object, what = "plant", xp))
  }
  class(res) <- "psHDM"
  return(res)
}
