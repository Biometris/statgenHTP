...
To illustrate, we will use the `spatCorrectedVator` data (available in the package), which contains information for all time points and a subset of genotypes on the spatially corrected photosystem II efficiency (`EffpsII_corr`). For simplicity, we will assume one population (`pop`).

```{r importspatCorrectedVator}
## Add population to data.
spatCorrectedVator$pop <- rep("Pop1", nrow(spatCorrectedVator))
```
...
fit.psHDM  <- fitSplineHDM(inDat = spatCorrectedVator,
                           ...
                           trait = "EffpsII_corr",
                           useTimeNumber = FALSE, 
                           # This means that the timePoint is going to be used instead of timeNumber
                           pop = "pop",
                           genotype = "genotype",
                           plotId = "plotId",
...
The `plot.psHDM()` function plots `psHDM` objects (as obtained from `fitSplineHDM()` and `predict.psHDM()` functions). In addition, if a `predict.psHDM` object is used, $95\%$ pointwise confidence intervals are depicted. To illustrate, we will use the prediction object (`pred.psHDM`).
...
```{r plotPredPopVator, fig.height=4, fig.width=7, message=FALSE, warning=FALSE}
...
```{r plotPredGenoTraVator, fig.height=4, fig.width=7, message=FALSE, warning=FALSE}
...
```{r plotPredGenoDerivVator, fig.height=4, fig.width=7, message=FALSE, warning=FALSE}
...
```{r plotPredGenoDevVator, fig.height=4, fig.width=7, message=FALSE, warning=FALSE}
...
```{r plotPredPlotVator, fig.height=4, fig.width=7, message=FALSE, warning=FALSE}
plot(pred.psHDM, 
...
```{r paramPhenoGenoPred, fig.height=2, fig.width=4, message=FALSE, warning=FALSE}
paramVator2 <-
