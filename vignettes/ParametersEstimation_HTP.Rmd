...
We will use the `fit.splineNumOut` previously created (see **statgenHTP tutorial: 4. Outlier detection Time courses**). It contains the p-spline prediction on a subset of plants without the time course outliers. We will estimate the maximum value of the trait at the end of the time course, so after the light change:
...
For this example, we will use the genotypic prediction (BLUPs, see **statgenHTP tutorial: 3. Correction for spatial trends**) available in the `spatPredArch` data set. We will fit Psplines at the genotypic level using the `geno.decomp` levels defined in the spatial model.
...
We can estimate the maximum value of the leaf area from the predicted psplines:
...
```{r paramArch1fig, fig.height=2, fig.width=5, echo=TRUE, message=FALSE, warning=FALSE, eval=TRUE}
...
For this example, we will use the raw data from the RootPhAir, corrected for time points outliers (see **statgenHTP tutorial: 2. Outlier detection Time points**) and time course (see **statgenHTP tutorial: 4. Outlier detection Time courses**). We will fit Psplines at the plant level on a subset of genotypes.
