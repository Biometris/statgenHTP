...
It was developped within the [EPPN^2020^ project](https://eppn2020.plant-phenotyping.eu/).  in response to demand for pipeline of analysis... blablabla joint work tested on several datasets... lablabla
...
Default engine is SpATS: has proven as good alternative to AR1xAR1 in the field and also suitable for platform, has been tested on several datasets in the EPPN2020 project.
...
When SpATS is used for modeling, an extra spatial term is included in the model. This spatial component is composed
...
can be modified using the `control` parameter in `fitTD`. Extra fixed effects may be fitted using the parameter `covariates`. The model specifications are listed in the table below.

otpion | model fitted | spatial term |
---------------- | ---------------------------------- | ------------------------- |
default | trait = *covariates* + **genotype** + $\epsilon$ | PSANOVA(colCoord, rowCoord, nseg = nSeg, nest.div = 2) |
covariates = c("A","B") | trait = *A* + *B* + **genotype** + $\epsilon$ | PSANOVA(colCoord, rowCoord, nseg = nSeg, nest.div = 2) |
useRepId = TRUE | trait = *covariates* + *repId* + **genotype** + **repId:rowId** + **repId:colId** +  $\epsilon$ | PSANOVA(colCoord, rowCoord, nseg = nSeg, nest.div = 2) |
useCheck = TRUE | trait = *covariates* + *check* + **genoCheck** +  $\epsilon$ | PSANOVA(colCoord, rowCoord, nseg = nSeg, nest.div = 2) |
...
time.
```{r plotSpSpat, message=FALSE, fig.height=5, fig.width=6}
plot(modPhenoSp, 
     plotType = "timeLapse")
```
...
```
By default, all the functions run for all the time point. It is possible to select some using `TimePoint`.
...
```{r plotSpPred, message=FALSE, fig.height=5, fig.width=6}
plot(modPhenoSp, 
     plotType = "rawPred",
...
```{r plotSpCorr, message=FALSE, fig.height=5, fig.width=6}
plot(modPhenoSp, 
     plotType = "corrPred",
     genotypes = c("col", "ler", "ely", "evo1") )
```
...
single plot.
```{r plotSpHerit, message=FALSE, fig.height=5, fig.width=6}
plot(modPhenoSp, 
     plotType = "herit")
```

Plots the effective dimension from models fitted using SpATS over time.
```{r plotSpED, message=FALSE, fig.height=5, fig.width=6}
plot(modPhenoSp, 
     plotType = "effDim")
```

Plots the residual, column and row variance for the fitted model over time.
```{r plotSpVar, message=FALSE, fig.height=5, fig.width=6}
plot(modPhenoSp, 
     plotType = "variance")
```
...
When asreml is used for modeling and spatial = TRUE six models are fitted with different random terms and covariance structure. The best model is determined based on a goodness-of-fit criterion, either AIC or BIC. This can be set using the control parameter criterion, default is AIC. The fitted random terms depend on the structure of the data. If the design has a regular structure, i.e. all replicates appear the same amount of times in the design, the following combinations of random and spatial terms are fitted

otpion | model fitted | spatial term |
---------------- | ------------------------------------- | ---------------------- |
spatial = TRUE, regular design | trait = **genotype** + *covariates* + $\epsilon$ | exp(rowCoord):colCoord |
spatial = TRUE, regular design | trait = **genotype** + *covariates* + $\epsilon$ | rowCoord:exp(colCoord) |
spatial = TRUE, regular design | trait = **genotype** + *covariates* + $\epsilon$ | iexp(rowCoord,colCoord) |
spatial = TRUE, regular design | trait = **genotype** + **repId:rowId** + *covariates* + $\epsilon$ | exp(rowCoord):colCoord |
spatial = TRUE, regular design | trait = **genotype** + **repId:colId** + *covariates* + $\epsilon$ | rowCoord:exp(colCoord) |
spatial = TRUE, regular design | trait = **genotype** + **repId:rowId** + *covariates* + $\epsilon$ | iexp(rowCoord,colCoord) |
spatial = TRUE, irregular design | trait = **genotype** + *covariates* + $\epsilon$ | ar1(rowId):colId |
spatial = TRUE, irregular design | trait = **genotype** + *covariates* + $\epsilon$ | rowId:ar1(colId) |
spatial = TRUE, irregular design | trait = **genotype** + *covariates* + $\epsilon$ | ar1(rowId):ar1(colId) |
spatial = TRUE, irregular design | trait = **genotype** + **repId:rowId** + *covariates* + $\epsilon$ | ar1(rowId):colId |
spatial = TRUE, irregular design | trait = **genotype** + **repId:colId** + *covariates* + $\epsilon$ | rowId:ar1(colId) |
spatial = TRUE, irregular design | trait = **genotype** + **repId:rowId** + *covariates* + $\epsilon$ | ar1(rowId):ar1(colId) |

## Modelling without spatial terms with ASReml without variance decomposition

When the row,column coordinate are not available.
