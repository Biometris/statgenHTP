...
```{r fitSpRCD, message=FALSE, eval=FALSE}
## Fit a model for a single time point.
...
genoPredSpCheck <- getGenoPred(modPhenoSpCheck, timePoints = 3)
# Extract the corrected values: 
spatCorrSp <- getCorrected(modPhenoSp, timePoints = 6)
spatCorrSpCheck <- getCorrected(modPhenoSpCheck, timePoints = 3)
...
## Fit a model on one time point with spatial function and check genotypes:
modPhenoSpAsCheck <- fitModels(TP = phenoTP, 
                               ...
                               engine = "asreml",
                               spatial = TRUE)
## Fit a model on one time point with spatial function and rowcol design:
modPhenoSpAsRCD <- fitModels(TP = phenoTP, 
                             useRepId = TRUE,
                             trait = "pheno",
                             timePoints = 3,
                             engine = "asreml",
...
# genoPredSpAs <- getGenoPred(modPhenoSpAs, timePoints = 6)
# genoPredSpAsCheck <- getGenoPred(modPhenoSpAsCheck, timePoints = 3)
```
...
Extracting results from the model works exactly the same as previously:

```{r extractGD, message=FALSE}
genoPredSpGD <- getGenoPred(modPhenoSpGD, timePoints = 6)
spatCorrSpGD <- getCorrected(modPhenoSpGD, timePoints = 6)
varianceSpGD <- getVar(modPhenoSpGD)
heritSpGD <- getHerit(modPhenoSpGD)
effDimSpGD <- getEffDims(modPhenoSpGD)

# genoPredSpAsGD <- getGenoPred(modPhenoSpAsGD, timePoints = 6)
# spatCorrSpAsGD <- getCorrected(modPhenoSpAsGD, timePoints = 6)
# varianceSpAsGD <- getVar(modPhenoSpAsGD)
# heritSpAsGD <- getHerit(modPhenoSpAsGD)
```

Prediction is now giving two values per genotype, one per level of geno.decomp, here "Basin", as illustrated in the table below for three genotypes predicted from the SpATS model `genoPredSpGD`.

```{r getPredSpGD, echo=FALSE, message=FALSE}
knitr::kable(head(genoPredSpGD[genoPredSpGD$genotype%in%c("1","1016","1060"),],6), 
             align=c('c','c','c','c','c','c'), padding = 0)
```

Plotting the results works also the same but some output will look different. The heritability plot now display one line per level of geno.decomp:

```{r plotSpHeritGD, message=FALSE, fig.height=2, fig.width=4, fig.align = 'center'}
plot(modPhenoSpGD, 
     plotType = "herit")
# plot(modPhenoSpAsGD, 
#      plotType = "herit")
...
```{r plotSprawGD, message=FALSE, fig.height=3, fig.width=6, fig.align = 'center'}
plot(modPhenoSpGD, 
     plotType = "rawPred",
     genotypes = c("col", "ler", "ely", "evo1") )
# plot(modPhenoSpAsGD, 
