...
It was developped within the EPPN^2020^ project.
in response to demand for pipeline of analysis...
blablabla joint work tested on several datasets... lablabla

<center>
[![logoEPPN](LogoEPPN_2020_pfade.jpg){width=10%}](https://eppn2020.plant-phenotyping.eu/)
</center>
...
The first example used in this vignette contains data from an experiment in the Phenovator 
...
```{r plotSpatRaw,  fig.height=4, fig.width=8, message=FALSE, eval=TRUE}
plot(modPhenoSp,
     timePoints = 36,
     plotType = "spatial",
     spaTrend = "raw")
```
For a straighforward interpretation of the fitted spatial trends, the plot can be 
displayed as a ratio of the raw phenotypic mean: SpatTrend(proportion) = Estimated SpatTrend / mean(raw pheno). 
In this case, the scale will be in percentage and the min/max will be adjusted based on 
all the time points used but will be at least 10%. This arbitrary threshold allow 
displaying fitted trends that have a relatively small to large importance.

```{r plotSpatPerc,  fig.height=4, fig.width=8, message=FALSE, eval=TRUE}
plot(modPhenoSp,
     timePoints = 36,
     plotType = "spatial",
     spaTrend = "percentage")
...
```
By default, all the functions run for all the time points. It is possible to select some of them using `timePoints`.
Effective dimensions ratio using `EDType = "ratio"`
...
plot(modPhenoSp, 
     plotType = "rawPred",
     genotypes = c("check1","check2","G7","G58")) #levels(factor(PhenovatorDat1$Genotype))[1:28] )
```
...
dimension for one component is large, it indicates that there are strong spatial trends 
in this direction. Here, for better comparison between component, we look at the ratio
of effective dimensions: 0 = no spatial trend, 1 = strong spatial trend (almost all 
the degrees of freedom are used to model it).
...
In phentyping platform, one can need to split the genotypic variance into different level. This is the case when, for example, a water scenario was randomly applied to the genotype. It is thus require to model a genotypic variance separately. 
In this part, we will describe briefly what are the specificities of such a modelling using the statgenHTP package using a second example dataset.

### Example 2

The second example used in this vignette contains data from an experiment in the 
Phenoarch platform (INRA, France, [@Cabrera2016]) with maize plants. It consists of 
...
Note that thereare more plants per replicates than one for the first panel 
...
The `phenoTParch` object just created is a `list` with 35 items, one for each time points 
 in the original `data.frame` (called `PhenoarchDat1`). We can visualise the layout an the 
...
the genotypes using the water scenaio:
...
Using the `geno.decomp` option when modelling will have two main consequences in the 
model: the geno.decomp variable(s) will be added as main fixed effect and also as 
interaction with the genotype effect. 
...
Extracting results and plotting from the models works exactly the same as previously. 
For example the spatial plot on this dataset look like that for one time point:

```{r plotSatASReml, echo=FALSE, message=FALSE, eval=TRUE, fig.height=4, fig.width=8, fig.align='center'}
plot(modPhenoSpGD,
     timePoints = 19,
     plotType = "spatial",
     spaTrend = "percentage")
...
The heritabilities are now split into geno.decomp level and their plot now displays
one line per level of geno.decomp.

```{r getHeritSpGD, echo=FALSE, message=FALSE, eval=TRUE}
genoHeritSpGD <- getHerit(modPhenoSpGD)
knitr::kable(head(genoHeritSpGD), 
             align=c('c','c','c','c'), padding = 0)
```

```{r plotSpHeritGD, echo=FALSE, fig.align='center', fig.height=3, fig.width=5, message=FALSE, eval=FALSE}
plot(modPhenoSpGD, 
     plotType = "herit",
     yLim = c(0.4,1))
```
...
```{r plotSprawGD, echo=FALSE, message=FALSE, fig.height=3, fig.width=7, fig.align = 'center', eval=TRUE}
plot(modPhenoSpGD, 
     plotType = "rawPred",
