...
>  *TO NOTE: in this example, plants are constantly moving and it was not possible to get unique timepoint containing the full platform. As a consequence, it is not possible to perform a spatial analysis per time point, but the other functions for outliers detection and longitudinal modelling are available and illustrated in the following tutorials.*
...
It is possible to transform a TP object back into a data frame with the `as.data.frame()` function.
