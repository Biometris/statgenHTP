...
5. Parameter extraction - *statgenHTP tutorial: 5. Estimation of parameters from time courses*  

This document describes in detail the three data sets used to example the functions. It also contains description on how to prepare the data for analysis and how to visualize them.
...
The tip root data set was obtained during an experiment performed at the [RootPhAir platform](https://uclouvain.be/en/research-institutes/eli/elia/aeroponics.html) (Louvain-La-Neuve University). This platform consists in two aeroponic tanks of 495 plants located in the same greenhouse. Plants are held on 5 plant strips with 99 strips per tank. Sprinklers are placed in the bottom of the tanks and spray a nutrient solution. Strips move constantly and plants are pictured when the strip passes in front of the camera. Plants are pictured every two hours. Root system is described in two dimensions, with the root tip position in depth and width (tipPos_y and tipPos_x respectively) deduced from image analysis.
...
>  *TO NOTE: in this example, plants are constantly moving and it was not possible to get unique time points containing the full platform. As a consequence, it is not possible to perform a spatial analysis per time point, but the other functions for outliers detection and longitudinal modeling are available and illustrated in the following tutorials. The parameters extracted from the curves can be modeled for spatial trends using the [statgenSTA package](https://biometris.github.io/statgenSTA/index.html).*
...
It is possible to visualize the raw data of a given trait on the layout, as a heatmap. 
...
Raw data can be displayed per genotype. By default all genotypes are used but it can be restricted using `genotypes` and a subset of genotypes. By default, data are plotted as points but this can be changed to lines by setting `plotLine = TRUE`.
