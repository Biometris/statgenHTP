...
>  *TO NOTE: In this platform, plants are constantly moving to be pictured. Hence it is not possible to get unique time points containing the full platform. As a consequence, it is not possible to perform a spatial analysis per time point, but the other functions for outliers detection and longitudinal modeling are available and illustrated in the following tutorials. The parameters extracted from the curves can be further modeled, to correct for example for other trends, like blocks, using the [statgenSTA package](https://biometris.github.io/statgenSTA/index.html).*
