...
* $f_t(u,v) + Z_r c_{rt} + Z_c c_{ct}$, represent the spatial effects, with $f_t(u,v)$ corresponding to the spatial function, e.g. the PSANOVA function from the SpATS package, $Z_r c_{rt}$ and $Z_c c_{ct}$ represent the design matrices and effects for the random row and column effects respectively, 
...
We will use the `TP` object `phenoTPOut` in which annotated time points were replaced by NA because they were considered to be outliers. (see [**statgenHTP tutorial: 2. Outlier detection Time points**](OutliersTimePoint_HTP.html)).
...
Different plots can be displayed for the `fitMod` object. The first one is **rawPred**, it plots the raw data (colored dots) overlaid with the predicted values (black points) from the fitted model. For each genotype a plot is made per plot/plant over time. These plots are put together in a 5×5 grid.   
