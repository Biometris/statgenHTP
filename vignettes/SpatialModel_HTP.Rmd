...
We will use the `TP` object `phenoTPOut` in which annotated time points have been replaced by NA (see [**statgenHTP tutorial: 2. Outlier detection Time points**](OutliersTimePoint_HTP.html)).
...
We will use the `TP` object **phenoTParchOut** in which annotated time points have been replaced by NA (see [**statgenHTP tutorial: 2. Outlier detection Time points**](OutliersTimePoint_HTP.html)).
...
In this case, time point "2017-05-14" has only 488 observation, less than half of the total number of observation. It can biased the modelling at this time point and one may want to remove it before running the spatial analysis. 
