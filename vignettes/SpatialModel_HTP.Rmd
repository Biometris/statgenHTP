...
Different plots can be displayed for the `fitMod` object. The first one is **rawPred**, it plots the raw data (colored dots, one color per plotId) overlaid with the predicted values (black points) from the fitted model. One plot is made per genotype with all its plotId. These plots are put together in a 5×5 grid per page.   
By using the option `genotypes`, a subset of genotypes will be plotted. By default, data are plotted as dot but this can be changed by setting `plotLine = TRUE`.
