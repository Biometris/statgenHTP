...
Different plots can be displayed for the `fitMod` object. The first one is **rawPred**, it plots the raw data (colored points) overlaid with the predicted values (black points) from the fitted model. For each genotype a plot is made per plot/plant over time. These plots are put together in a 5×5 grid.   
...
The second one is **corrPred**, it plots the spatially corrected data (colored points) overlaid with the predicted values from the fitted model (black points). For each genotype a plot is made per plot/plant over time. These plots are put together in a 5×5 grid. 
By using the option `genotypes`, a subset of genotypes will be plotted. By default, data are plotted as point but this can be changed by setting `plotLine = TRUE`.
...
Note that when check genotypes are used for modeling, for the two previous plot types (`rawPred` and `corrPred`), the option `plotChecks` is required to display check genotypes.
