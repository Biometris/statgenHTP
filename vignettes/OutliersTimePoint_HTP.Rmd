...
We are then running the function on a subset of plant to set the parameters, using the `plotIds` option. The smoothing parameter `mylocfit` is set at 0.1. The curve are atypical and to detect only the very strange point, fitted curves have to be not too smooth. The confidence interval size `confIntSize` is set at 3 which is a relatively narrow interval.
```{r setParamVator}
